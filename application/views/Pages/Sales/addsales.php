            ...
            <div class="modal-content">
              ...
              <div class="modal-body">
                <div class="form-group">
                  ...
                  <input type="password" class="form-control" id="password_check" placeholder="Password">
                </div>
              </div>
              <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                <button type="button" id="submit_pass" class="btn btn-primary">Submit</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="row well well-sm">
          ...
          <div class="col-sm-3">
            <div class="form-group">
              ...
              <input type="hidden" id="temp_modal" name="temp_modal" class="form-control text-right curency" value="0" data-parsley-vprice="" required="">
              <input type="hidden" class="form-control" id="password_cache">
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    $('#submit_pass').click(function(e){
      ...
      $.ajax({
        type: "POST",
        url: "<?php echo base_url(); ?>Sales/check_pass",
        dataType: "json",
        data: {password:password},
        success : function(data){
          if (data.code == "200"){
            $("#password_cache").val(password);
            $('#password').modal('hide');
          } else {
            Swal.fire({
              icon: 'error',
  ...
  $('#product_name').autocomplete({ 
    ...
    select: function(event, ui) {
      ...
      $('#temp_modal').val(ui.item.modal_price);
