...
<div class="content-wrapper">
  ...
  <section class="content">
    ...
    <div class="card">
      <div class="card-header">
        <h3 class="card-title">List Retur Pembelian</h3>

        <div class="card-tools">  
          <?php if($data['check_role'][0]->add_ac == 'Y'){?> 
            <button type="button" class="btn btn-primary" id="add_returpurcchase"><i class="fas fa-plus"></i> Tambah</button>
          <?php }else{ ?>
           <button type="button" class="btn btn-primary" id="add_returpurcchase" disabled><i class="fas fa-plus"></i> Tambah</button>
         <?php } ?>
     ...
     <div class="card-body">
      <table id="example" class="table table-bordered" style="width:100%">
        ...
        <tbody>
          <?php foreach($data['retur_purchase_list'] as $row){ ?>
            <tr>
              <td><?php echo $row->hd_retur_purchase_invoice; ?></td>
          ...
          <td>
            <?php if($data['check_role'][0]->view_ac == 'Y'){?>
              <button class="btn btn-sm btn-primary" onclick="detail('<?php echo $row->hd_retur_purchase_id; ?>')"><i class="fas fa-eye"></i></button>
            <?php }else{ ?>
              <button class="btn btn-sm btn-primary" onclick="detail('<?php echo $row->hd_retur_purchase_id; ?>')" disabled><i class="fas fa-eye"></i></button>
            <?php } ?>
            <?php if($data['check_role'][0]->delete_ac == 'Y'){?>
              <button class="btn btn-sm btn-danger" onclick="deletes('<?php echo $row->hd_retur_purchase_id; ?>', '<?php echo $row->hd_retur_purchase_invoice; ?>')"><i class="fas fa-trash"></i></button>
            <?php }else{ ?>
              <button class="btn btn-sm btn-danger" onclick="deletes('<?php echo $row->hd_retur_purchase_id; ?>', '<?php echo $row->hd_retur_purchase_invoice; ?>')" disabled><i class="fas fa-trash"></i></button>
            <?php } ?>
          </td>
        </tr>
      <?php } ?>
  ...
  function deletes(id, name){
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          type: "POST",
          url: "<?php echo base_url(); ?>Retur/delete_retur_purchase",
          dataType: "json",
          data: {id:id},
...
<script type="text/javascript">
  $(document).ready(function() {
    $('#add_returpurcchase').click(function(e){
      e.preventDefault();
      window.location.href = '<?php echo base_url(); ?>Retur/addreturpurchase';
    });
  });
